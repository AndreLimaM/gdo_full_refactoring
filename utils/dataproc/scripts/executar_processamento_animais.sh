...
# Script para executar o processamento de dados de animais usando Dataproc e Cloud SQL
...
exibir_ajuda() {
    ...
    echo "  --table-name TABELA    Nome da tabela para gravar os dados (padrão: animais)"
    echo "  --mode MODO            Modo de gravação: append ou overwrite (padrão: append)"
    echo "  --ajuda                Exibe esta mensagem de ajuda"
    echo ""
    echo "Exemplo:"
    echo "  $0 --projeto meu-projeto --bucket meu-bucket --input-path gs://meu-bucket/animais/*.json --db-host 10.0.0.1 --db-password minhasenha"
    exit 1
}
...
if [ -z "$PROJETO" ] || [ -z "$BUCKET" ] || [ -z "$INPUT_PATH" ] || [ -z "$DB_HOST" ] || [ -z "$DB_PASSWORD" ]; then
...
echo "Iniciando processamento de animais..."
...
SCRIPT_PATH="$(dirname "$(readlink -f "$0")")/processar_animais.py"
gsutil cp "$SCRIPT_PATH" "gs://$BUCKET/datalake/utils/dataproc/scripts/"

# Executar o job no Dataproc
echo "Executando job de processamento de animais..."
gcloud dataproc jobs submit pyspark "gs://$BUCKET/datalake/utils/dataproc/scripts/processar_animais.py" \
    --region="$REGIAO" \
    --project="$PROJETO" \
    --cluster="$CLUSTER" \
    -- \
...
# Verificar resultado
if [ $? -eq 0 ]; then
    echo "Processamento de animais concluído com sucesso!"
else
    echo "Erro durante o processamento de animais."
    exit 1
fi
