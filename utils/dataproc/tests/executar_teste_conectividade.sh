...
echo -e "${VERDE}Executando teste de conectividade entre Dataproc e Cloud SQL...${NC}"
...
gsutil ls gs://${BUCKET_NAME}/scripts/install_postgresql_driver.sh &> /dev/null
if [ $? -ne 0 ]; then
    echo -e "${AMARELO}Script de inicializau00e7u00e3o nu00e3o encontrado. Enviando para o bucket...${NC}"
    gsutil cp ./utils/dataproc/install_postgresql_driver.sh gs://${BUCKET_NAME}/scripts/install_postgresql_driver.sh
    if [ $? -ne 0 ]; then
...
# Enviar o script de teste para o bucket
echo -e "${AMARELO}Enviando script de teste para o bucket...${NC}"
gsutil cp ./utils/dataproc/teste_conectividade_basica.py gs://${BUCKET_NAME}/jobs/teste_conectividade_basica.py
if [ $? -ne 0 ]; then
    echo -e "${VERMELHO}Erro ao enviar o script de teste.${NC}"
    exit 1
fi
echo -e "${VERDE}Script de teste enviado com sucesso.${NC}"

# Criar o cluster Dataproc (versu00e3o mais simples)
...
# Executar o job
echo -e "${AMARELO}Executando teste de conectividade...${NC}"
gcloud dataproc jobs submit pyspark \
    gs://${BUCKET_NAME}/jobs/teste_conectividade_basica.py \
...
if [ ${TESTE_RESULT} -ne 0 ]; then
    echo -e "${VERMELHO}Teste de conectividade falhou.${NC}"
    exit 1
fi

echo -e "${VERDE}Teste de conectividade concluu00eddo.${NC}"
