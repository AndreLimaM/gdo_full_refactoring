...
gsutil cp ./utils/dataproc/testar_conexao_simples.py gs://${BUCKET_NAME}/jobs/testar_conexao_simples.py
if [ $? -ne 0 ]; then
...
# Executar o job
echo -e "${AMARELO}Executando teste de conexu00e3o...${NC}"
gcloud dataproc jobs submit pyspark \
    gs://${BUCKET_NAME}/jobs/testar_conexao_simples.py \
...
if [ ${TESTE_RESULT} -ne 0 ]; then
    echo -e "${VERMELHO}Teste de conexu00e3o falhou.${NC}"
    exit 1
fi

echo -e "${VERDE}Teste de conexu00e3o concluu00eddo com sucesso.${NC}"
